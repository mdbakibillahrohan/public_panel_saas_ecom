<template>
  <div class="relative">
    ...
    <div class="h-20"></div>
    <header class="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 fixed top-0 left-0 right-0 z-50">
      <div class="mx-auto px-4 container py-3">
        <div class="flex items-center justify-between gap-4 md:gap-6">
          ...
          <NuxtLink to="/" class="flex items-center space-x-3 group cursor-pointer shrink-0">
            <div class="relative">
              <div
                class="absolute inset-0 bg-primary rounded-lg blur-md opacity-50 group-hover:opacity-75 transition-opacity">
              </div>
              <div
                class="relative bg-gradient-to-br from-primary to-blue-600 p-2 rounded-lg shadow-lg group-hover:scale-110 transition-transform duration-300">
          ...
          <div class="flex-1 max-w-2xl hidden md:block">
            <div class="relative group">
              <input v-model="searchQuery" type="text" placeholder="Search for products, brands and more..."
              ...
              <span
                class="material-symbols-outlined absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400 text-xl">search</span>
            </div>
          </div>
          ...
          <nav class="flex items-center gap-1 md:gap-2">
            <!-- Desktop Nav Links -->
            <NuxtLink v-for="item in menusItems" :key="item.name" :to="item.link"
              ...
              <span
                class="absolute inset-0 bg-gradient-to-r from-blue-500/0 via-blue-400/10 to-blue-500/0 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity"></span>
            </NuxtLink>
            ...
            <template v-if="isAuthenticated">
              <UDropdownMenu :items="userMenuItems" :popper="{ placement: 'bottom-end' }">
                <button
                  class="flex items-center gap-2 px-2 py-1.5 rounded-lg text-white/70 hover:text-white hover:bg-white/10 transition-all">
                  <div
                    class="w-8 h-8 rounded-full bg-gradient-to-br from-primary to-blue-600 flex items-center justify-center text-white text-sm font-bold ring-2 ring-white/20">
                    {{ initials }}
                  </div>
            ...
            <template v-else>
              <NuxtLink to="/auth/sign-in">
                <button
                  class="hidden sm:flex items-center gap-2 px-4 py-2 rounded-lg text-white/80 hover:text-white hover:bg-white/10 font-medium transition-all">
                  Sign In
                </button>
              </NuxtLink>
              <NuxtLink to="/auth/sign-up">
                <button
                  class="hidden sm:flex items-center gap-2 px-4 py-2 rounded-lg bg-primary hover:bg-blue-600 text-white font-bold shadow-lg shadow-blue-500/30 transition-all">
                  Sign Up
                </button>
              </NuxtLink>
